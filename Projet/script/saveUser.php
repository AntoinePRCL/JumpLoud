	...
	if( count($_POST) == 12
		...
		//Vérifier les champs un par un
			...
			if($error){
				...
				$query = $connection->prepare("INSERT INTO user (gender,email,nom,prenom, password,date_naissance,pseudo,country,ville, validationKey)
					VALUES ( :gender, :email, :nom, :prenom,:password, :date_naissance, :pseudo, :country,:ville, :validationKey) ");	
				$validationKey = substr(sha1($_POST['email']),4,10);		
				$query->execute([
									"gender"=>$_POST["gender"],
									"email"=>$_POST["email"],
									"nom"=>$_POST["lastname"],
				...
				$message = "Bienvenue sur JumpLoud, voici votre code de validation : ".$validationKey." vous pouvez également cliquer ici pour valider votre compte : http://http://localhost/Projet/accountActivation.php?key=".$validationKey."&email=".$_POST['email'];
				mail($_POST["email"], 'Confirmation d\'inscription JumpLoud', $message);
